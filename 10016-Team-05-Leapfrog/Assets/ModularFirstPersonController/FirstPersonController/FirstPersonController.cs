...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void HeadBob()
    {
        ...
        if (isWalking)
        {
            // Update the timer based on the current movement state
            UpdateTimer();

            // Calculate the new position for head bobbing based on the timer
            Vector3 newPosition = CalculateHeadBobPosition();
        ...
        {
            // Reset timer and smoothly transition joint position back to original
            ResetHeadBob();
        }
    }

    // Updates the timer based on current movement state
    ...
    // Calculates the new head bob position
    private Vector3 CalculateHeadBobPosition()
    {
        float xBob = Mathf.Sin(timer) * bobAmount.x;
        float yBob = Mathf.Sin(timer) * bobAmount.y;
        float zBob = Mathf.Sin(timer) * bobAmount.z;

        return new Vector3(jointOriginalPos.x + xBob, jointOriginalPos.y + yBob, jointOriginalPos.z + zBob);
    }

    // Resets the head bob effect when the player stops moving
